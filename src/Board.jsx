...
const CardSelectionModal = ({ targetPlayer, targetHand, onConfirm, onCancel, title, singleSelection, revealHand, showHand = true, showEquip = true, showJudges = true, imageMode = 'url' }) => {
...
const ChengxiangBox = ({ data, isOwner, onToggle, onConfirm, onCancel, imageMode = 'url' }) => {
  if (!data || !data.active) return null;
  const cards = Array.isArray(data.revealedCards) ? data.revealedCards : [];
  const selectedIds = Array.isArray(data.selectedIds) ? data.selectedIds : [];
  const sum = typeof data.sum === 'number' ? data.sum : 0;
  const canConfirm = isOwner && selectedIds.length > 0 && sum <= 13;
  return (
    <div style={{
      position: 'fixed',
      top: 0, left: 0, right: 0, bottom: 0,
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', color: '#27ae60' }}>
          <h3 style={{ margin: 0 }}>称象</h3>
          <div style={{ fontWeight: 'bold', color: sum > 13 ? '#e74c3c' : '#ecf0f1' }}>点数和 {sum}</div>
        </div>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '10px', justifyContent: 'center' }}>
          {cards.map((card, index) => {
            const id = card && card.id !== undefined ? String(card.id) : `${card?.name || 'card'}-${index}`;
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '10px' }}>
          <button
            ...
            style={{
              padding: '8px 20px',
              backgroundColor: isOwner ? '#95a5a6' : '#555',
              color: 'white',
              border: 'none',
              borderRadius: '4px',
          ...
          <button
            ...
            style={{
              padding: '8px 20px',
              backgroundColor: canConfirm ? '#2ecc71' : '#555',
              color: 'white',
              border: 'none',
              borderRadius: '4px',
...
const QiHuiModal = ({ qiHui, onSelectOption, onToggleButton, onConfirm }) => {
  const selectedOption = qiHui?.selectedOption ?? null;
  const litButtons = Array.isArray(qiHui?.litButtons) ? qiHui.litButtons : [];
  const canConfirm = !!selectedOption && litButtons.length === 1;
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ marginTop: '5px', fontSize: '12px', color: '#bdc3c7', textAlign: 'center' }}>
          请点灭两个按钮（保留一个标记）
        </div>

        <div style={{ display: 'flex', gap: '6px', justifyContent: 'center' }}>
          {['基本', '锦囊', '装备'].map(btn => {
            const isLit = litButtons.includes(btn);
            return (
              <button
                key={btn}
                onClick={() => onToggleButton(btn)}
                style={{
                  fontSize: '10px',
                  padding: '4px 8px',
        ...
        <button
          ...
          style={{
            padding: '10px',
            cursor: canConfirm ? 'pointer' : 'not-allowed',
            backgroundColor: canConfirm ? '#e67e22' : '#7f8c8d',
            border: 'none',
            color: 'white',
            borderRadius: '5px',
            fontWeight: 'bold'
    ...
    >
      ...
      {typeof daoxinValue === 'number' && (
        <div style={{
          ...
          {(onDaoXinAdd5 || onDaoXinAdd15) && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', marginBottom: '6px' }}>
              {onDaoXinAdd5 && (
                <button
                  onClick={(e) => { e.stopPropagation(); onDaoXinAdd5(); }}
                  style={{
                    width: '22px',
                    height: '16px',
                    padding: 0,
              ...
              {onDaoXinAdd15 && (
                <button
                  onClick={(e) => { e.stopPropagation(); onDaoXinAdd15(); }}
                  style={{
                    width: '22px',
                    height: '16px',
                    padding: 0,
          ...
          <div style={{
            ...
            <div style={{
              ...
              right: 0,
              bottom: 0,
              height: `${Math.max(0, Math.min(100, Math.round((daoxinValue / 99) * 100)))}%`,
              background: 'linear-gradient(180deg, rgba(46, 204, 113, 0.95) 0%, rgba(46, 204, 113, 0.65) 100%)'
            }} />
            {[25, 50, 75, 100].map(mark => (
              <div key={mark} style={{
                ...
                <div style={{ width: '8px', height: '1px', backgroundColor: 'rgba(255,255,255,0.65)' }} />
                <div style={{ fontSize: '9px', color: 'rgba(255,255,255,0.85)', lineHeight: '9px' }}>{mark}</div>
              </div>
            ))}
          </div>
          <div style={{ marginTop: '6px', fontSize: '10px', color: 'rgba(255,255,255,0.9)', fontWeight: 'bold' }}>
            {daoxinValue}
          </div>
        </div>
      )}
      ...
      {jiuAnimKey > 0 && (
        <div
          ...
          style={{
            ...
            borderRadius: '12px',
            pointerEvents: 'none',
            background: 'radial-gradient(circle at 50% 70%, rgba(255, 120, 0, 0.55) 0%, rgba(255, 60, 0, 0.25) 40%, rgba(0,0,0,0) 70%)',
            boxShadow: '0 0 14px rgba(255, 120, 0, 0.8), 0 0 30px rgba(255, 40, 0, 0.55)',
            mixBlendMode: 'screen',
            border: '1px solid rgba(255, 160, 60, 0.75)'
          }}
        />
      )}
...
const TaoluanSelectVirtualModal = ({ usedNames, onSelect, onCancel }) => {
  ...
  const renderList = (names) => (
    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', justifyContent: 'center' }}>
      {names.filter(n => !used.has(n)).map((name) => (
        <button
          key={name}
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <h3 style={{ margin: 0 }}>滔乱：请选择声明牌名</h3>
...
const SuitPickModal = ({ title, items, onPick, onCancel, imageMode = 'url' }) => {
  return (
    <div style={{
      position: 'absolute',
      top: 0,
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
          {(items || []).length === 0 && (
            <div style={{ textAlign: 'center', color: '#bdc3c7' }}>暂无可选花色</div>
          )}
          {(items || []).map(it => (
            <div key={it.suit} style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              <div style={{ display: 'flex', justifyContent: 'center' }}>
                <button
                  onClick={() => onPick && onPick(it.suit)}
                  style={{
                    padding: '8px 14px',
                    backgroundColor: '#34495e',
                    color: 'white',
                ...
                >
                  {it.suit}{typeof it.count === 'number' ? `(${it.count})` : ''}
                </button>
              </div>
              <div style={{ display: 'flex', justifyContent: 'center', gap: '8px', flexWrap: 'wrap' }}>
                {(it.cards || []).map((card, idx) => (
                  <CardPictureButton
                    key={card && card.id ? card.id : `${it.suit}-${idx}`}
                    card={card}
                    width={60}
                    height={90}
                    onClick={() => onPick && onPick(it.suit)}
...
const OptionPickModal = ({ title, options, onPick, onCancel }) => {
  return (
    <div style={{
      position: 'absolute',
      top: 0,
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          {(options || []).map(opt => (
            <button
              key={opt.value}
              onClick={() => onPick && onPick(opt.value)}
              style={{
                padding: '10px 12px',
                backgroundColor: '#34495e',
                color: 'white',
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <h3 style={{ margin: 0 }}>滔乱：请选择要交出的牌</h3>
        {equipEntries.length > 0 && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            <div style={{ fontWeight: 'bold' }}>装备区</div>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '10px', justifyContent: 'center' }}>
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'center', gap: '20px' }}>
             <button 
                onClick={() => selectedIndex !== null && onConfirm(cards[selectedIndex]?.selection)} 
                disabled={selectedIndex === null}
...
export function CardBoard({ ctx, G, moves, playerID, userId, roomId, enableLobby }) {
  ...
  const onHeroClick = (targetId) => {
    ...
    if (G.caomaoQingzheng && G.caomaoQingzheng.active && G.caomaoQingzheng.stage === 'select_target' && G.caomaoQingzheng.sourceID === playerID) {
      ...
      moves.caomaoQingzhengSelectTarget(targetId);
      return;
    }
    ...
    if (G.caomaoFangzhu && G.caomaoFangzhu.active && G.caomaoFangzhu.stage === 'select_target' && G.caomaoFangzhu.sourceID === playerID) {
      ...
      moves.caomaoFangzhuSelectTarget(targetId);
      return;
    }
    ...
    if (G.taoluan && G.taoluan.active && G.taoluan.stage === 'after_choose_other' && G.taoluan.sourceID === playerID) {
      ...
      moves.taoluanSelectOther(targetId);
      return;
    }
  ...
  const handlePlayCards = () => {
    ...
    const updateFenYinIfNeeded = (card) => {
      const me = G.players[playerID];
      if (!me || !me.general || me.general.name !== '留赞' || !card) return;
      let message = '';
    ...
    updateFenYinIfNeeded(G.hands[playerID][selectedCardIndices[0]]);

    if (!pojunAutoPrompt && selectedCardIndices.length === 1) {
    ...
    if (!tieqiAutoPrompt && selectedCardIndices.length === 1) {
      const cardIndex = selectedCardIndices[0];
      const card = G.hands[playerID][cardIndex];
      const isSlash = card && ['杀', '火杀', '雷杀'].includes(card.name);
      const me = G.players[playerID];
    ...
    if (me.general && me.general.name === '留赞' && fenYinMessage && fenYinMessage.startsWith('奋音')) {
      setFenYinDrawPending(true);
    } else {
      setFenYinDrawPending(false);
    }
    setSelectedCardIndices([]);
    setSelectedTargetIds([]);
  };

  const resolvePoJunAutoPrompt = (shouldUsePoJun) => {
    if (!pojunAutoPrompt) return;
    const { cardIndices, targetIds } = pojunAutoPrompt;
    if (shouldUsePoJun && targetIds && targetIds.length > 0) {
      moves.usePoJun(targetIds[0]);
    }
    moves.playCards(cardIndices, targetIds);
    setSelectedCardIndices([]);
    setSelectedTargetIds([]);
    setPojunAutoPrompt(null);
  };

  const resolveTieQiAutoPrompt = (shouldUseTieQi) => {
    if (!tieqiAutoPrompt) return;
    const { cardIndices, targetIds } = tieqiAutoPrompt;
    if (shouldUseTieQi && targetIds && targetIds.length > 0) {
      moves.moumachaoStartMouyi(targetIds[0]);
    }
    moves.playCards(cardIndices, targetIds);
    setSelectedCardIndices([]);
    setSelectedTargetIds([]);
  ...
  const getSuitItems = (cards) => {
    const suits = ['♥', '♦', '♠', '♣'];
    const buckets = suits.map(suit => ({ suit, cards: [] }));
    (cards || []).forEach(c => {
      const bucket = buckets.find(b => b.suit === c?.suit);
      if (bucket) bucket.cards.push(c);
    });
    return buckets
      .filter(b => b.cards.length > 0)
      .map(b => ({ suit: b.suit, count: b.cards.length, cards: b.cards }));
  };

  const qingzheng = G.caomaoQingzheng;
  const qingzhengSelfItems = qingzheng && qingzheng.active && qingzheng.stage === 'select_suit_self'
    ? getSuitItems(G.hands[playerID] || [])
    : [];
  const qingzhengTargetItems = qingzheng && qingzheng.active && qingzheng.stage === 'select_suit_target' && qingzheng.targetID
    ? getSuitItems(G.hands[qingzheng.targetID] || [])
    : [];
  ...
  const onSkillClick = (rawSkillName) => {
    ...
    if (skillName === '清正') {
      moves.caomaoQingzhengStart();
      return;
    }

    if (skillName === '酒诗') {
      moves.caomaoJiuPoemActivate();
      return;
    }

    if (skillName === '放逐') {
      moves.caomaoFangzhuStart();
      return;
    }
    ...
    if (skillName === '应援') {
      const me = G.players[playerID];
      if (me && me.general && me.general.name === '马良') {
        if (G.maliang && G.maliang.status === 'selecting_transfer_target') {
          moves.maliangTransferCancel();
        } else {
          moves.maliangTransferStart();
        }
      }
      return;
    }
  ...
  const renderPlayerArea = (id) => {
    ...
    if (general && general.name === '曹髦') {
      const daoxin = typeof player.daoxin === 'number' ? player.daoxin : 0;
      displaySkills = ['潜龙'];
      if (daoxin >= 25) displaySkills.push('清正');
      if (daoxin >= 50) displaySkills.push('酒诗');
    ...
    if (general && general.name === '留赞') {
      ...
      if (fenyinLabel) {
        displaySkills = displaySkills.map(s => (s === '奋音' ? fenyinLabel : s));
      }
    }
    if (isMe && general) {
      if (general.name === '许攸') {
      ...
      if (general.name === '界钟会') {
        ...
        displaySkills = general.skills.map(s => {
          if (s === '权计') return `权计(${quanCount})`;
          return s;
        });
      }

      if (general.name === '李傕') {
        const yiSuanCardId = player.yiSuanLastTrickCardId;
        const yiSuanCard = yiSuanCardId ? G.discardPile.find(c => c && c.id === yiSuanCardId) : null;
        displaySkills = general.skills.map(s => {
          if (s === '亦算' && yiSuanCard && yiSuanCard.type === '锦囊') {
            return `亦算(${yiSuanCard.suit} ${yiSuanCard.rank} ${yiSuanCard.name})`;
          }
          return s;
        });
      }
    ...
    const qingzhengSelecting = G.caomaoQingzheng && G.caomaoQingzheng.active && G.caomaoQingzheng.stage === 'select_target' && G.caomaoQingzheng.sourceID === playerID;
    const fangzhuSelecting = G.caomaoFangzhu && G.caomaoFangzhu.active && G.caomaoFangzhu.stage === 'select_target' && G.caomaoFangzhu.sourceID === playerID;
    const isSelectable = selectedCardIndices.length > 0 || mizhaoSelecting || !!activeSkill || yimouSelecting || huishiSelecting || chongzhenSelecting || qingzhengSelecting || fangzhuSelecting;
    ...
    const isCaomao = general && general.name === '曹髦';
    const daoxinValue = isCaomao ? (typeof player.daoxin === 'number' ? player.daoxin : 0) : null;
    const onDaoXinAdd5 = isMe && isCaomao ? () => moves.caomaoAddDaoxin(5) : null;
    const onDaoXinAdd15 = isMe && isCaomao ? () => moves.caomaoAddDaoxin(15) : null;
        ...
        <React.Fragment key={id}>
          <div style={areaStyle}>
            ...
            <div style={{ 
              ...
              {general && general.name === '界吕布' && selectedCardIndices.length === 1 && selectedTargetIds.length === 1 && !G.liyuTargeting?.active && !G.liyuCardSelecting?.active && (() => {
                const card = hand[selectedCardIndices[0]];
                return card && ['杀', '火杀', '雷杀'].includes(card.name);
              })() && (
                <button
                  onClick={() => {
                    moves.useLiyu();
                    moves.selectLiyuTarget(selectedTargetIds[0]);
                    moves.confirmLiyuTarget();
                  }}
                  style={{
                    padding: '8px 20px',
                    backgroundColor: '#f1c40f',
              ...
              {general && general.name === '留赞' && fenYinDrawPending && selectedCardIndices.length === 0 && (
                <button
                  data-fenyin-draw-btn="true"
                  onClick={() => {
                    setFenYinDrawPending(false);
                    moves.liuzanFenyinDraw();
                  }}
                  style={{
                    padding: '8px 20px',
                    backgroundColor: '#e74c3c',
              ...
              {G.taoluan && G.taoluan.active && G.taoluan.stage === 'select_targets' && G.taoluan.sourceID === playerID && (
                (() => {
                  ...
                  return (
                    <>
                      {hint && <div style={{ color: '#f1c40f', fontWeight: 'bold', alignSelf: 'center' }}>滔乱：{hint}</div>}
                      <button
                        onClick={() => {
                          const targets = needsZero ? [] : selectedTargetIds;
                          moves.taoluanConfirmPlay(targets);
                          setSelectedTargetIds([]);
                        }}
                        disabled={!ok}
                        style={{
              ...
              {G.taoluan && G.taoluan.active && G.taoluan.stage === 'waiting_resolution' && G.taoluan.sourceID === playerID && (
                <>
                  <div style={{ color: '#f1c40f', fontWeight: 'bold', alignSelf: 'center' }}>滔乱：请先结算该牌效果</div>
                  <button
                    onClick={() => moves.taoluanCancel()}
                    style={{
                      padding: '8px 20px',
                      backgroundColor: '#95a5a6',
                      color: 'white',
              ...
              {G.taoluan && G.taoluan.active && G.taoluan.stage === 'after_choose_other' && G.taoluan.sourceID === playerID && (
                <>
                  <div style={{ color: '#f1c40f', fontWeight: 'bold', alignSelf: 'center' }}>滔乱：请选择一名其他角色</div>
                  <button
                    onClick={() => moves.taoluanCancel()}
                    style={{
                      padding: '8px 20px',
  ...
  return (
    <div
      onClickCapture={(e) => {
        if (!fenYinDrawPending) return;
        const target = e.target;
        if (target && typeof target.closest === 'function' && target.closest('[data-fenyin-draw-btn="true"]')) return;
        setFenYinDrawPending(false);
      }}
      style={{ 
      width: '100vw', 
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ fontSize: '12px', color: '#bdc3c7', textAlign: 'center' }}>
              你使用了【{pojunAutoPrompt.cardName}】指定 {G.players[pojunAutoPrompt.targetIds?.[0]]?.general?.name || `Player ${pojunAutoPrompt.targetIds?.[0]}`} 为目标
            </div>
            <div style={{ display: 'flex', justifyContent: 'center', gap: '12px' }}>
              <button
                onClick={() => resolvePoJunAutoPrompt(true)}
                style={{
                  padding: '8px 18px',
                  backgroundColor: '#2ecc71',
                  color: 'white',
              ...
              <button
                onClick={() => resolvePoJunAutoPrompt(false)}
                style={{
                  padding: '8px 18px',
                  backgroundColor: '#7f8c8d',
                  color: 'white',
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'center', gap: '12px' }}>
              <button
                onClick={() => {
                  moves.jiezhonghuiQuanJiAutoStart();
                  setQuanjiAutoPrompt(false);
                }}
                style={{
                  padding: '8px 18px',
                  backgroundColor: '#2ecc71',
              ...
              <button
                onClick={() => setQuanjiAutoPrompt(false)}
                style={{
                  padding: '8px 18px',
                  backgroundColor: '#7f8c8d',
                  color: 'white',
      ...
      {tieqiAutoPrompt && (
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ fontSize: '12px', color: '#bdc3c7', textAlign: 'center' }}>
              你使用了【{tieqiAutoPrompt.cardName}】指定 {G.players[tieqiAutoPrompt.targetIds?.[0]]?.general?.name || `Player ${tieqiAutoPrompt.targetIds?.[0]}`} 为目标
            </div>
            <div style={{ display: 'flex', justifyContent: 'center', gap: '12px' }}>
              <button
                onClick={() => resolveTieQiAutoPrompt(true)}
                style={{
                  padding: '8px 18px',
                  backgroundColor: '#2ecc71',
                  color: 'white',
              ...
              <button
                onClick={() => resolveTieQiAutoPrompt(false)}
                style={{
                  padding: '8px 18px',
                  backgroundColor: '#7f8c8d',
                  color: 'white',
      ...
      {G.mouyi && G.mouyi.active && (G.mouyi.sourcePlayerID === playerID || G.mouyi.targetPlayerID === playerID) && (
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ fontSize: '12px', color: '#bdc3c7', textAlign: 'center' }}>
              你与对手同时选择；若相同则不执行，若不同则谋马超按其选择执行
            </div>
            {(() => {
              const isSource = G.mouyi.sourcePlayerID === playerID;
              const myChoice = isSource ? G.mouyi.sourceChoice : G.mouyi.targetChoice;
              if (myChoice) {
                return (
                  <div style={{ textAlign: 'center', color: '#f1c40f', fontWeight: 'bold' }}>
                    已选择，等待对方选择…
              ...
              return (
                <div style={{ display: 'flex', justifyContent: 'center', gap: '12px', flexWrap: 'wrap' }}>
                  <button
                    onClick={() => moves.moumachaoChooseMouyi('steal')}
                    style={{
                      padding: '10px 14px',
                      backgroundColor: '#3498db',
                      color: 'white',
                  ...
                  <button
                    onClick={() => moves.moumachaoChooseMouyi('draw')}
                    style={{
                      padding: '10px 14px',
                      backgroundColor: '#e67e22',
                      color: 'white',
      ...
      {G.caoangAutoPrompt && G.caoangAutoPrompt.active && G.caoangAutoPrompt.playerID === playerID && (
        <div style={{
          position: 'fixed',
          top: 0, left: 0, right: 0, bottom: 0,
          backgroundColor: 'rgba(0,0,0,0.6)',
          ...
          <div style={{
            ...
            <div style={{ fontSize: '12px', color: '#bdc3c7', textAlign: 'center' }}>
              {G.players[G.caoangAutoPrompt.slashSourceID]?.general?.name || `Player ${G.caoangAutoPrompt.slashSourceID}`} 使用了【{G.caoangAutoPrompt.cardName || '杀'}】，
              目标为 {G.players[G.caoangAutoPrompt.slashTargetID]?.general?.name || `Player ${G.caoangAutoPrompt.slashTargetID}`}
            </div>
            <div style={{ fontSize: '12px', color: '#f1c40f', textAlign: 'center' }}>
              确定后你先摸一张牌，再选择目标交牌
            </div>
            <div style={{ display: 'flex', justifyContent: 'center', gap: '12px' }}>
              <button
              ...
              <button
                onClick={() => moves.caoangAutoPromptCancel()}
                style={{
                  padding: '8px 18px',
                  backgroundColor: '#7f8c8d',
                  color: 'white',
      ...
      {G.players[playerID] && G.players[playerID].qiHui && G.players[playerID].qiHui.stage === 'selecting_option' && (
         <QiHuiModal
           qiHui={G.players[playerID].qiHui}
           onSelectOption={(opt) => moves.youxushuQiHuiSelectOption(opt)}
           onToggleButton={(btn) => moves.youxushuQiHuiClick(btn)}
           onConfirm={() => moves.youxushuQiHuiConfirm()}
         />
      )}
      ...
      {showShiCaiModal && (
        <ShiCaiModal
            ...
            onMoveToTop={(index) => {
              moves.xuyouShiCaiToTop(index);
              setShowShiCaiModal(false);
            }}
      ...
      {qingzheng && qingzheng.active && qingzheng.stage === 'select_suit_self' && qingzheng.sourceID === playerID && (
        <SuitPickModal
          title="清正：选择弃置花色"
          items={qingzhengSelfItems}
          onPick={(s) => moves.caomaoQingzhengSelectSuitSelf(s)}
          onCancel={() => moves.caomaoQingzhengCancel()}
          imageMode={cardImageMode}
        />
      )}

      {qingzheng && qingzheng.active && qingzheng.stage === 'select_target' && qingzheng.sourceID === playerID && (
        <SimplePromptModal
          title="清正：请选择目标"
          onCancel={() => moves.caomaoQingzhengCancel()}
          isBlocking={false}
      ...
      {qingzheng && qingzheng.active && qingzheng.stage === 'select_suit_target' && qingzheng.sourceID === playerID && (
        <SuitPickModal
          title="清正：选择目标弃置花色"
          items={qingzhengTargetItems}
          onPick={(s) => moves.caomaoQingzhengSelectSuitTarget(s)}
          onCancel={() => moves.caomaoQingzhengCancel()}
          imageMode={cardImageMode}
        />
      )}

      {G.caomaoFangzhu && G.caomaoFangzhu.active && G.caomaoFangzhu.stage === 'select_target' && G.caomaoFangzhu.sourceID === playerID && (
        <SimplePromptModal
          title="放逐：请选择目标"
          onCancel={() => moves.caomaoFangzhuCancel()}
          isBlocking={false}
        />
      )}

      {G.caomaoFangzhu && G.caomaoFangzhu.active && G.caomaoFangzhu.stage === 'select_option' && G.caomaoFangzhu.sourceID === playerID && (
        <OptionPickModal
          title="放逐：选择效果"
          options={[
            { value: 'disable_skills', label: '技能失效直到其下回合结束' },
            { value: 'forbid_nontrick_hand', label: '不能使用除锦囊牌以外的手牌直到其下回合结束' }
          ]}
          onPick={(v) => moves.caomaoFangzhuSelectOption(v)}
          onCancel={() => moves.caomaoFangzhuCancel()}
        />
      )}
      ...
      {G.taoluan && G.taoluan.active && G.taoluan.stage === 'select_card' && G.taoluan.sourceID === playerID && (
        <TaoluanSelectMaterialModal
          hand={G.hands[playerID]}
          equipments={G.players[playerID]?.equipments}
      ...
      {G.taoluan && G.taoluan.active && G.taoluan.stage === 'select_virtual' && G.taoluan.sourceID === playerID && (
        <TaoluanSelectVirtualModal
          usedNames={G.taoluanGlobalUsedNames}
          onSelect={(name) => {
            setSelectedTargetIds([]);
      ...
      {G.taoluan && G.taoluan.active && G.taoluan.stage === 'other_choose_option' && G.taoluan.otherID === playerID && (
        <TaoluanOtherChoiceModal
          onGive={() => moves.taoluanOtherChoose(1)}
          onPunish={() => moves.taoluanOtherChoose(2)}
        />
      )}

      {G.taoluan && G.taoluan.active && G.taoluan.stage === 'other_give_card' && G.taoluan.otherID === playerID && (
        <TaoluanGiveCardModal
          hand={G.hands[playerID]}
          equipments={G.players[playerID]?.equipments}
      ...
      {activeSkill && (
        <div style={{
          ...
          {activeSkill === '狼袭' && selectedTargetIds.length > 0 && (
            <button
              onClick={() => {
                if (selectedTargetIds.length !== 1) {
                  alert("请选择一名目标");
                ...
                moves.lijueLangXi(selectedTargetIds[0]);
                setActiveSkill(null);
                setSelectedTargetIds([]);
              }}
              style={{
          ...
          {activeSkill === '饮战' && selectedTargetIds.length > 0 && (
            <button
              onClick={() => {
                if (selectedTargetIds.length !== 1) {
                  alert("请选择一名目标");
                ...
                moves.startYinZhan(selectedTargetIds[0]);
                setActiveSkill(null);
                setSelectedTargetIds([]);
              }}
              style={{
      ...
      {G.selectCard && G.selectCard.active && G.selectCard.sourcePlayerID === playerID && (
        <CardSelectionModal
          ...
          targetHand={G.selectCard.onlyEquip ? [] : G.hands[G.selectCard.targetPlayerID]}
          ...
          showHand={!G.selectCard.onlyEquip}
          showJudges={!G.selectCard.onlyEquip}
          imageMode={cardImageMode}
        />
      )}
      ...
      {G.caocongChengxiang && G.caocongChengxiang.active && (
        <ChengxiangBox
          data={G.caocongChengxiang}
          isOwner={G.caocongChengxiang.sourcePlayerID === playerID}
          onToggle={(cardId) => moves.caocongChengxiangToggle(cardId)}
          onConfirm={() => moves.caocongChengxiangConfirm()}
          onCancel={() => moves.caocongChengxiangCancel()}
          imageMode={cardImageMode}
        />
      )}
