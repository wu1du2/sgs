...
import { sunluyuSkill } from './skills/sunluyu.js';
...
const applyDamageInternal = (G, random, targetPlayerID, damage) => {
  ...
  if (remaining > 0) {
    ...
    if (player.general && player.general.name === '曹髦') {
      caomaoInternal.addDaoXin(G, targetPlayerID, remaining * 10, '受伤');
      if (player.is_turned_over) {
        player.is_turned_over = false;
        caomaoInternal.gainTrickLikeRangjie(G, random, targetPlayerID);
        G.actionLog.push(`曹髦 受到伤害后翻回正面`);
      }
    }
  }
  clampArmorUtil(player);
};
...
const createPlayerState = () => ({
  ...
  shitaishiciZhanLieX: 0,
  shitaishiciHanZhan: 'hpMax',
  shitaishiciZhanLie: 'attackRange',
  userId: null,
  ...createEmptyZones()
});
...
const drawCards = (G, playerID, count, rng) => {
  ...
  addCardsToHand(G, playerID, cardsToDraw);
  return cardsToDraw;
};
...
const activateLiegongInternal = (G, random, playerID, targetID) => {
  const player = G.players[playerID];
  if (!player || !player.general || player.general.name !== '谋黄忠') return;
  ...
  for (let i = 0; i < revealCount; i++) {
    const card = drawTopCard(G, random);
...
const playCardsInternal = ({ G, playerID, random }, cardIndices, targetIds) => {
  ...
  if (G.players[playerID].general && G.players[playerID].general.name === '留赞' && cardsPlayed.length > 0) {
      const firstCard = cardsPlayed[0];
      let currentColor = null;
      if (['♥', '♦'].includes(firstCard.suit)) currentColor = 'red';
      else if (['♠', '♣'].includes(firstCard.suit)) currentColor = 'black';
  ...
  // Liu Yan - Tushe
  if (G.players[playerID].general && G.players[playerID].general.name === '刘焉') {
      const firstCard = cardsPlayed[0];
      // Check if non-equip and has targets
      const isEquip = ['武器', '防具', '加一', '减一'].includes(firstCard.type);
      if (!isEquip && targetIds && targetIds.length > 0) {
          // Check if hand is empty
          if (hand.length === 0) {
              // Ask user? skills_info says "Ask user if 1/2/3/Cancel".
              // But for now, let's just draw automatically or use a simple prompt if needed.
              // skills_info says "若无手牌则摸X张牌".
              // Let's draw X.
              const count = targetIds.length;
              drawCards(G, playerID, count, random);
              const playerName = G.players[playerID].general ? G.players[playerID].general.name : `Player ${playerID}`;
              G.actionLog.push(`${playerName} 发动图射，摸 ${count} 张牌`);
          }
      }
  }
  ...
  if (G.players[playerID].general && G.players[playerID].general.name === '骆统') {
      ...
      const logs = luotongSkill.qinzheng.trigger(G, playerID, random);
...
export const CardGame = {
  setup: ({ random }) => ({
    deck: shuffle(SGS_CARDS.map((c, i) => ({ ...c, id: `card-${i}` })), random),
    discardPile: [],
    hands: {
      '0': [],
      '1': [],
    ...
    lostCardPlayers: [], // Track players who lost cards this turn
    selectCard: {
      active: false,
      sourcePlayerID: null,
      targetPlayerID: null,
  ...
  turn: {
    ...
    onBegin: ({ G, ctx }) => {
        ...
        // You Xushu Init
        if (!G.youxushuInitialized) {
            Object.keys(G.players).forEach(pid => {
                const player = G.players[pid];
                if (player.general && player.general.name === '友徐庶') {
                    // Check if already equipped (to avoid double equip if onBegin runs multiple times improperly)
                    // But checking initialized flag handles it.
                    youxushuSkill.moves.youxushuXiaXing({ G, playerID: pid });
                }
            });
            G.youxushuInitialized = true;
        }
    },
  },
  ...
  moves: {
    ...
    ...sunluyuSkill.moves,
    ...liuyanSkill.moves,
    shitaishiciUpdateZhanLieX: ({ G, playerID }, x) => {
      const player = G.players[playerID];
      if (!player || !player.general || player.general.name !== '势太史慈') return;
      player.shitaishiciZhanLieX = x;
    },
    shitaishiciUpdateZhenFeng: ({ G, playerID }, hanZhan, zhanLie) => {
      const player = G.players[playerID];
      if (!player || !player.general || player.general.name !== '势太史慈') return;
      player.shitaishiciHanZhan = hanZhan;
      player.shitaishiciZhanLie = zhanLie;
    },
    baoxinMutao: ({ G, ctx, playerID, random }, targetID) => {
      baoxinSkill.mutao.action({ G, ctx, playerID, random }, targetID);
    },
    baoxinYimouOptionOne: ({ G, ctx, playerID, random }, targetID) => {
      baoxinSkill.yimou.optionOne({ G, ctx, playerID, random }, targetID);
    },
    ...
    baoxinYimouConfirmCard: ({ G, ctx, playerID, random }, selected) => {
      baoxinSkill.yimou.confirmCard({ G, ctx, playerID, random }, selected);
    },
    baoxinYimouCancel: ({ G, playerID }) => {
      baoxinSkill.yimou.cancel({ G, playerID });
    },
    performJudgment: ({ G, playerID, random }) => {
      // Draw judgment card
      if (G.deck.length === 0) {
        if (G.discardPile.length > 0) {
    ...
    drawCard: ({ G, playerID, random }, actionId) => {
      const player = G.players[playerID];
    ...
    taoluanConfirmPlay: ({ G, playerID, random }, targetIDs) => {
      const t = G.taoluan;
      ...
      if (t.material.type === 'hand') {
        ...
        addCardsToHand(G, playerID, fromEquip);
      ...
      if (playCardsMove) {
        playCardsMove({ G, playerID, random }, [cardIndex], resolvedTargets);
      }
    ...
    taoluanOtherChoose: ({ G, playerID }, option) => {
      const t = G.taoluan;
    ...
    taoluanOtherGiveCard: ({ G, playerID }, selected) => {
      ...
      addCardsToHand(G, t.sourceID, card);
    ...
    duyuMiewuConfirmPlay: ({ G, playerID, random }, targetIDs) => {
      const m = G.miewu;
      ...
      const drawn = drawCards(G, playerID, 1, random);
      if (drawn.length > 0) {
    ...
    shenguojiaHuishiJudge: ({ G, playerID, random }) => {
      const h = G.huishi;
    ...
    shenguojiaHuishi2Choose: ({ G, playerID, random }, option) => {
      const h = G.huishi2;
    ...
    shenguojiaZuoxingConfirmPlay: ({ G, playerID, random }, targetIDs) => {
      const z = G.zuoxing;
    ...
    rangjieChooseOption: ({ G, playerID, random }, option) => {
      if (option === 'cancel') {
    ...
    useLuckCard: ({ G, playerID, random }, actionId) => {
      const player = G.players[playerID];
    ...
    playerReady: ({ G, playerID, random }) => {
      ...
      if (G.readyPlayers.length === 3 && G.phase === 'lobby') {
        ...
        const { generalOptions, generalChangeUsed } = distributeGenerals(random);
        G.generalOptions = generalOptions;
        G.generalChangeUsed = generalChangeUsed;
      }
    },
    selectGeneral: ({ G, playerID, random }, generalId) => {
      ...
      if (selected) {
        ...
        clampArmorUtil(G.players[playerID]);
        ...
        if (selected.name === '曹髦') {
          const p = G.players[playerID];
          p.daoxin = 20;
          p.is_turned_over = false;
          p.caomaoFangzhuUsed = false;
          p.caomaoFangzhuLastTarget = null;
          p.caomaoJuejinUsed = false;
          p.grantedSkills = [];
          caomaoInternal.checkUnlocks(p);
          G.actionLog.push(`曹髦 获得20点道心，当前${p.daoxin}/99`);
        }
      }
    ...
    modifyHP: ({ G, random }, targetPlayerID, amount) => {
      const player = G.players[targetPlayerID];
      if (player && player.hp !== undefined) {
        if (amount > 0) {
           ...
           const damage = -amount;
           if (damage > 0 && !(G.pendingEffect && G.pendingEffect.active) && !(G.selectCard && G.selectCard.active)) {
             const target = G.players[targetPlayerID];
             if (target && target.hp === 1) {
               let remainingAfterArmor = damage;
               const armor = typeof target.armor === 'number' ? target.armor : 0;
               if (armor > 0) {
                 remainingAfterArmor = Math.max(0, remainingAfterArmor - armor);
               }
               if (remainingAfterArmor <= 0) {
                 applyDamageInternal(G, random, targetPlayerID, damage);
                 return;
               }
               const caocongID = Object.keys(G.players || {}).find(pid => {
                 if (pid === targetPlayerID) return false;
                 const p = G.players[pid];
                 if (!p || !p.general || p.general.name !== '曹冲') return false;
                 if (!p.equipments) return false;
                 return Object.values(p.equipments).some(Boolean);
               });
               if (caocongID) {
                 G.pendingEffect = {
                   active: true,
                   sourcePlayerID: caocongID,
                   targetPlayerID: targetPlayerID,
                   actionType: 'caocong_renxin',
                   pendingCard: { name: '仁心' },
                   damage: damage
                 };
                 return;
    ...
    playCardToJudgment: ({ G, playerID, random }, { card, targetPlayerID, type }) => {
    ...
    playCards: ({ G, playerID, random }, cardIndices, targetIds) => {
      playCardsInternal({ G, playerID, random }, cardIndices, targetIds);
    },
    ...
    shuffleDeck: ({ G, random }) => {
      ...
      G.deck = shuffle(combined, random);
      G.discardPile = [];
    ...
    mizhaoConfirmTargetA: ({ G, playerID }, targetAID) => {
      ...
      addCardsToHand(G, targetAID, movedCards);
    ...
    triggerHarvest: ({ G, playerID, random }) => {
      // Draw X cards where X is number of players (3)
      const numPlayers = 3;
      const cards = drawCards(G, playerID, numPlayers, random);
    ...
    // Jie Zhonghui Skills
    jiezhonghuiQuanJi: ({ G, playerID, random }) => {
        // Draw 1 card first
        drawCards(G, playerID, 1, random);
    ...
    jiezhonghuiQuanJiAutoStart: ({ G, playerID, random }) => {
        drawCards(G, playerID, 1, random);
        G.jiezhonghuiQuanJiSelect = {
            active: true,
            playerID: playerID
    ...
    // Jie Li Ru Skills
    jieliruUseJuece: ({ G, playerID, random }, targetID) => {
        jieliruSkill.useJuece({ G, playerID }, targetID, (G, pid, tid, dmg) => applyDamageInternal(G, random, tid, dmg));
    },
    jieliruUseFencheng: ({ G, playerID }) => {
        jieliruSkill.useFencheng({ G, playerID });
    },
    jieliruFenchengRespond: ({ G, playerID, random }, choice, cardsToDiscard) => {
        ...
        if (playerID !== currentTargetID) return;

        const player = G.players[playerID];
        
        if (choice === 'damage') {
             applyDamageInternal(G, random, playerID, 2);
             G.actionLog.push(`${player.general.name} 受到 2 点伤害（焚城）`);
        } else if (choice === 'discard') {
             const handIndices = [];
             const equipSlots = [];
             const judgeSlots = [];
             
             cardsToDiscard.forEach(s => {
                 if (s.type === 'hand') handIndices.push(s.index);
                 else if (s.type === 'equip') equipSlots.push(s.slot);
                 else if (s.type === 'judge') judgeSlots.push(s.slot);
             });
             
             const cards = [];
             handIndices.sort((a, b) => b - a).forEach(idx => {
                 if (G.hands[playerID][idx]) {
                    cards.push(G.hands[playerID].splice(idx, 1)[0]);
                 }
             });
             equipSlots.forEach(slot => {
                 if (G.players[playerID].equipments[slot]) {
                    cards.push(G.players[playerID].equipments[slot]);
                    G.players[playerID].equipments[slot] = null;
                 }
             });
             judgeSlots.forEach(slot => {
                 if (G.players[playerID].judges[slot]) {
                    cards.push(G.players[playerID].judges[slot]);
                    G.players[playerID].judges[slot] = null;
                 }
             });
             
             if (cards.length > 0) {
                cards.forEach(c => addToDiscardPile(G, c));
                
                if (!G.lostCardPlayers.includes(playerID)) G.lostCardPlayers.push(playerID);
                state.currentX = cards.length + 1;
                G.actionLog.push(`${player.general.name} 弃置了 ${cards.length} 张牌`);
             }
        }
    ...
    activateKangkaiWithDraw: ({ G, playerID, random }) => {
        drawCards(G, playerID, 1, random);
        caoangSkill.kangkai.activate({ G, playerID });
        const playerName = G.players[playerID].general ? G.players[playerID].general.name : `Player ${playerID}`;
    ...
    caoangAutoPromptAccept: ({ G, playerID, random }) => {
    ...
    moumachaoChooseMouyi: ({ G, playerID, random }, choice) => {
        if (!G.mouyi || !G.mouyi.active) return;
        ...
        if (sourceChoice === 'draw') {
          drawCards(G, sourcePlayerID, 2, random);
    ...
    // Cao Chun Skills
    confirmShanjia: ({ G, playerID, random }, discardCount) => {
        // Draw 3 cards
        drawCards(G, playerID, 3, random);
    ...
    useChouJue: ({ G, playerID, random }) => {
        wenyangSkill.useChouJue({ G, playerID }, (G, pid, count) => drawCards(G, pid, count, random));
    },
    ...
    mouhuangzhongPlayCardsWithLiegong: ({ G, playerID, random }, { cardIndices, targetIds, useLiegong }) => {
        playCardsInternal({ G, playerID, random }, cardIndices, targetIds);
        if (useLiegong && targetIds && targetIds.length > 0) {
            activateLiegongInternal(G, random, playerID, targetIds[0]);
        }
    },
    mouhuangzhongActivateLiegong: ({ G, playerID, random }, targetID) => {
        activateLiegongInternal(G, random, playerID, targetID);
    },
    ...
    startYinZhan: ({ G, playerID }, targetID) => {
        shiweiyanSkill.startYinZhan({ G, playerID }, targetID);
    },
        ...
        if (actionType === 'steal') {
            ...
            if (selected.length > 0) {
                ...
                if (card) {
                    ...
                    // Track lost cards
                    if (!G.lostCardPlayers.includes(targetID)) {
                        G.lostCardPlayers.push(targetID);
                    }
                }
            }
            ...
            if (selected.length > 0) {
                const item = selected[0];
                if (G.selectCard.onlyEquip && item.type !== 'equip') return;
                const targetPlayer = G.players[targetID];
                const targetHand = G.hands[targetID];
                let card = null;
                ...
                if (card) {
                    ...
                    const effectName = G.selectCard && G.selectCard.pendingCard && G.selectCard.pendingCard.name ? G.selectCard.pendingCard.name : '过河拆桥';
    ...
    confirmEffect: ({ G, playerID }) => {
        ...
        const { actionType, targetPlayerID, pendingCard, damage } = G.pendingEffect;
        ...
        if (actionType === 'steal' || actionType === 'discard' || actionType === 'collateral') {
             ...
             const source = G.players[playerID];
             if (source) source.caomaoJuejinUsed = true;

             Object.keys(G.players || {}).forEach(pid => {
               const p = G.players[pid];
               if (!p) return;
               const oldHp = typeof p.hp === 'number' ? p.hp : 0;
               const reduced = Math.max(0, oldHp - 1);
               p.hp = 1;
               const baseArmor = typeof p.armor === 'number' ? p.armor : 0;
               const extra = pid === playerID ? 2 : 0;
               p.armor = baseArmor + reduced + extra;
               clampArmorUtil(p);
             });

             G.caomaoJuejinAuraActive = true;

             const exiled = [];
             const sweepArray = (arr) => {
               if (!Array.isArray(arr) || arr.length === 0) return;
               for (let i = arr.length - 1; i >= 0; i--) {
                 const c = arr[i];
                 if (isJuejinForbiddenCard(c)) {
                   exiled.push(arr.splice(i, 1)[0]);
                 }
               }
             };

             sweepArray(G.deck);
             sweepArray(G.discardPile);
             Object.keys(G.hands || {}).forEach(pid => sweepArray(G.hands[pid]));
             Object.keys(G.players || {}).forEach(pid => {
               const p = G.players[pid];
               if (!p) return;
               ['weapon', 'armor', 'plusOne', 'minusOne'].forEach(slot => {
                 const c = p.equipments && p.equipments[slot];
                 if (isJuejinForbiddenCard(c)) {
                   exiled.push(c);
                   p.equipments[slot] = null;
                 }
               });
               ['bing', 'le', 'dian'].forEach(slot => {
                 const c = p.judges && p.judges[slot];
                 if (isJuejinForbiddenCard(c)) {
                   exiled.push(c);
                   p.judges[slot] = null;
                 }
               });
               if (Array.isArray(p.quan)) sweepArray(p.quan);
               if (p.pojun && typeof p.pojun === 'object') {
                 Object.keys(p.pojun).forEach(k => sweepArray(p.pojun[k]));
               }
             });
             if (Array.isArray(G.harvestCards)) sweepArray(G.harvestCards);
             if (G.rangjieTempCard && isJuejinForbiddenCard(G.rangjieTempCard)) {
               exiled.push(G.rangjieTempCard);
               G.rangjieTempCard = null;
             }
             if (exiled.length > 0) {
               pushToExiled(G, exiled);
             ...
             G.actionLog.push(`曹髦 发动决进：全场体力调整为1并移除【闪】【桃】【酒】`);
    ...
    cancelEffect: ({ G, playerID, random }) => {
        if (!G.pendingEffect || !G.pendingEffect.active || G.pendingEffect.sourcePlayerID !== playerID) return;
        if (G.pendingEffect.actionType === 'caocong_renxin' && typeof G.pendingEffect.damage === 'number') {
          applyDamageInternal(G, random, G.pendingEffect.targetPlayerID, G.pendingEffect.damage);
        }
    ...
    changeGeneral: ({ G, playerID, random }, generalId, actionId, clickid, sessionid) => {
      // Idempotency check
      ...
      if (available.length > 0) {
        ...
        const randIndex = Math.floor(random.Number() * available.length);
        const newGeneral = available[randIndex];
    ...
    voteRematch: ({ G, playerID, random }) => {
      ...
      if (G.rematchVotes.length === 3) {
        ...
        // Reset G
        const newG = CardGame.setup({ random });
    ...
    selectJianyingName: ({ G, ctx, playerID, random }, newName) => {
        if (!G.jianyingSelect.active || G.jianyingSelect.playerID !== playerID) return;
        ...
        if (card && cardIndex !== -1) {
            ...
            CardGame.moves.playCards({ G, ctx, playerID, random }, [cardIndex], []);
    ...
    // Liu Yan Skills (Moved to src/skills/liuyan.js)
    ...
    selectLongHunCard: ({ G }, cardData) => {
         if (G.longhunSelect.active && G.longhunSelect.stage === 'card_selection') {
             G.longhunSelect.selectedCard = cardData;
         }
    },
    confirmLongHunCard: ({ G }, directCard) => {
        // Allow passing card directly to avoid race conditions with selectLongHunCard
        const selectedCard = directCard || G.longhunSelect.selectedCard;
        const { sourcePlayerID, targetPlayerID } = G.longhunSelect;
        
        if (!selectedCard) return;

        const { type, index, slot, id } = selectedCard;
        ...
        if (type === 'hand') {
            // Prioritize ID match if available to avoid race conditions
            if (id) {
                const foundIndex = targetHand.findIndex(c => c.id === id);
                if (foundIndex !== -1) {
                    cardToDiscard = targetHand.splice(foundIndex, 1)[0];
                }
            } else if (index >= 0 && index < targetHand.length) {
                cardToDiscard = targetHand.splice(index, 1)[0];
            }
        } else if (type === 'equip') {
            // Verify ID for equipment if available
            if (id && targetPlayer.equipments[slot] && targetPlayer.equipments[slot].id !== id) {
                 // Equipment changed, abort
                 return;
            }
    ...
    useKurou: ({ G, playerID }) => {
        const player = G.players[playerID];
        if (!player || !player.general || player.general.name !== '谋黄盖') {
            // Fallback for standard Huang Gai if general name differs or just in case
            if (player && player.general && player.general.name === '黄盖') {
                 player.hp = Math.max((player.hp || 0) - 1, 0);
                 // Need random here, but it's passed in args. I'll fix args below.
                 // But wait, the original useKurou had random. I should keep it in signature.
                 return;
            }
            return;
        }
        
        G.mouhuanggaiKurouSelect = {
            active: true,
            stage: 'card_selection',
            sourcePlayerID: playerID,
            targetPlayerID: null,
        ...
        G.actionLog.push(`谋黄盖 发动苦肉，请选择一张牌交给其他角色`);
    },
    
    // Standard Huang Gai Kurou (kept for compatibility if needed, but the button calls useKurou)
    // Actually the button calls useKurou. I need to handle both or update useKurou.
    // I updated useKurou above to check name.
    // But I need to add the standard one back if I want to support standard Huang Gai.
    // Standard Huang Gai logic: Lose 1 HP, Draw 2.
    // I will refactor useKurou to handle both.

    selectKurouCard: ({ G, playerID }, cardData) => {
        if (!G.mouhuanggaiKurouSelect || !G.mouhuanggaiKurouSelect.active || G.mouhuanggaiKurouSelect.sourcePlayerID !== playerID) return;
        if (G.mouhuanggaiKurouSelect.stage !== 'card_selection') return;
        
        G.mouhuanggaiKurouSelect.selectedCard = cardData;
        G.mouhuanggaiKurouSelect.stage = 'target_selection';
        G.actionLog.push(`请选择一名其他角色作为目标`);
    },

    selectKurouTarget: ({ G, playerID }, targetID) => {
        if (!G.mouhuanggaiKurouSelect || !G.mouhuanggaiKurouSelect.active || G.mouhuanggaiKurouSelect.sourcePlayerID !== playerID) return;
        if (G.mouhuanggaiKurouSelect.stage !== 'target_selection') return;
        if (targetID === playerID) return;

        G.mouhuanggaiKurouSelect.targetPlayerID = targetID;
        // Proceed to confirm/execute
        // Note: We cannot call CardGame.moves directly inside a move function in boardgame.io logic.
        // We must call the logic directly or trigger it.
        // Since confirmKurou is a move, we can just copy its logic or extract it to a helper function.
        // Or we can rely on the client to call confirmKurou.
        // But here we want auto-execution.
        // So I will invoke the logic directly.
        
        const state = G.mouhuanggaiKurouSelect;
        const source = G.players[playerID];
        const target = G.players[state.targetPlayerID];
        const selection = state.selectedCard;
        
        let card = null;
        let isPeachOrWine = false;

        if (selection.type === 'hand') {
            card = G.hands[playerID].splice(selection.index, 1)[0];
        } else if (selection.type === 'equip') {
            card = source.equipments[selection.slot];
            source.equipments[selection.slot] = null;
        }

        if (card) {
            addCardsToHand(G, state.targetPlayerID, card);
            isPeachOrWine = (card.name === '桃' || card.name === '酒');
            
            const sourceName = source.general ? source.general.name : `Player ${playerID}`;
            const targetName = target.general ? target.general.name : `Player ${state.targetPlayerID}`;
            G.actionLog.push(`${sourceName} 将 ${card.name} 交给 ${targetName}`);
            
            // Lose HP
            const hpLoss = isPeachOrWine ? 2 : 1;
            source.hp = Math.max(0, source.hp - hpLoss);
            G.actionLog.push(`${sourceName} 失去 ${hpLoss} 点体力`);
            
            // Gain Armor
            source.armor = (source.armor || 0) + 2;
            clampArmorUtil(source);
            G.actionLog.push(`${sourceName} 获得 2 点护甲`);
        }

        G.mouhuanggaiKurouSelect = null;
    },

    confirmKurou: ({ G, playerID }) => {
        const state = G.mouhuanggaiKurouSelect;
        if (!state || !state.active || state.sourcePlayerID !== playerID) return;
        ...
        if (card) {
            ...
            // Lose HP
            const hpLoss = isPeachOrWine ? 2 : 1;
            // Apply HP loss (direct subtraction as it is "lose hp" not "damage")
            // But usually "lose hp" triggers nothing, whereas "damage" triggers skills.
            // Kurou says "lose hp".
            source.hp = Math.max(0, source.hp - hpLoss);
            G.actionLog.push(`${sourceName} 失去 ${hpLoss} 点体力`);
    ...
    selectChouceCard: ({ G }, cardData) => {
         if (G.chouceSelect.active && G.chouceSelect.stage === 'card_selection') {
             G.chouceSelect.selectedCard = cardData;
         }
    },
    confirmChouceCard: ({ G }, directCard) => {
        // Allow passing card directly
        const selectedCard = directCard || G.chouceSelect.selectedCard;
        const { sourcePlayerID, targetPlayerID } = G.chouceSelect;
